...
import { TOOLTIP_NOTE } from "constants/ThemeColors";
...
const OnGoingMonitoringDialog = ({ onClose, selected, setSelected }) => {
  ...
  const handleSave = async () => {
    ...
    const data = await dispatch(
      KYC_ACTION_BULK_TOGGLE_OM({ kycIds, enabled: om })
    );
    ...
    if (errorFound.length) {
      ...
      snackActions.success(
        <FormattedHTMLMessage
          id={
            om
              ? "kyc.dialog.enabledOnGoingMonitoring.success"
              : "kyc.dialog.disabledOnGoingMonitoring.success"
          }
