...
import { TEXT_GREEN } from "constants/ThemeColors";

/**
 * component new design
 */
  ...
  return (
    <>
      ...
      <Grid item xs={4}>
        <div className={"d-flex flex-column"}>
          ...
          <div className={clsx(styles.gridItem, "d-flex align-items-center")}>
            <Typography
              variant="Subtitle1"
              color={getNameUserAction(data) && TEXT_GREEN}
            >
              <Nullable>{getNameUserAction(data)}</Nullable>
            </Typography>
          </div>
        </div>
      </Grid>
      <Grid item xs={4}>
        ...
        <div className={clsx(styles.gridItem, "d-flex align-items-center")}>
          <Typography variant="Subtitle1">
            <Nullable>{formatDate(data?.updatedAt, LONG_DATE_TIME)} </Nullable>
          </Typography>
        </div>
      </Grid>
      {data?.payload?.notes && (
...
const ActivityLog = ({ isOpen, onClose, refId, title, cleanState, id }) => {
